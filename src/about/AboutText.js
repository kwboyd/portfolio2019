import React from 'react';

const AboutText = () => {
    return (
        <div className="about-text content-column">
            <h3 data-testid="intro">Hi! I'm a full-stack developer in Denver, CO. I currently work on the Deploys team at Shopify.</h3>
            <p>I've built a mobile app from scratch with thousands of users that later was acquired by a larger gaming company.</p>
            <p>I've built a website that won multiple awards for its interactive storytelling.</p>
            <p>I have a degree in Journalism & Japanese. I like creative writing, dancing, cats, coffee, and JavaScript.</p>
        </div>
    )
}

export default AboutText;